...
function VariantesForm({ variantes, setVariantes }: { variantes: any[], setVariantes: (v: any[]) => void }) {
  function agregar() {
    setVariantes([...variantes, { color: '', talla: '', precio: '', precio_descuento: '', stock: '0' }])
  }
  function actualizar(index: number, campo: string, valor: string) {
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-bold text-sm">Variantes ({variantes.length})</h3>
        <button type="button" onClick={agregar} className="text-green-400 hover:text-green-300 text-sm font-bold">+ Agregar variante</button>
      </div>
      {variantes.length === 0 && <p className="text-gray-500 text-xs">Sin variantes. El producto usará el precio base.</p>}
      {variantes.map((v, i) => (
        <div key={i} className="bg-gray-900 rounded-lg p-3 space-y-2 border border-gray-700">
          <div className="flex justify-between items-center">
            <span className="text-gray-400 text-xs font-bold">Variante {i + 1}</span>
            <button type="button" onClick={() => eliminar(i)} className="text-red-400 hover:text-red-300 text-xs">✕ Eliminar</button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="Color" value={v.color} onChange={e => actualizar(i, 'color', e.target.value)}
            ...
            <input placeholder="Talla" value={v.talla} onChange={e => actualizar(i, 'talla', e.target.value)}
              className="bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-500" />
          </div>
          <div className="grid grid-cols-3 gap-2">
            <input placeholder="Precio *" type="number" value={v.precio} onChange={e => actualizar(i, 'precio', e.target.value)}
              className="bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-500" required />
            <input placeholder="Precio desc." type="number" value={v.precio_descuento} onChange={e => actualizar(i, 'precio_descuento', e.target.value)}
              className="bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-500" />
            <input placeholder="Stock" type="number" value={v.stock} onChange={e => actualizar(i, 'stock', e.target.value)}
              className="bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-500" />
          </div>
        </div>
      ))}
...
export default function AdminPage() {
  ...
  const [imagenEditar, setImagenEditar] = useState<any>(null)
  const [variantesEditar, setVariantesEditar] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    if (t) {
      ...
      if (a) {
        try { setAdminInfo(JSON.parse(a)) } catch {}
      }
      cargarConfig(t)
      cargarProductos(t)
      cargarCategorias(t)
  ...
  async function agregarProducto(e: any) {
    ...
    if (variantes.length > 0) {
      form.append('variantes', JSON.stringify(variantes))
    }
    ...
    if (res.ok) {
      ...
      setCategoriaId(''); setMarcaId(''); setVariantes([])
  ...
  function iniciarEdicion(p: any) {
    ...
    const vars = Array.isArray(p.variantes) ? p.variantes.map((v: any) => ({
      color: v.color || '', talla: v.talla || '', precio: v.precio || '', precio_descuento: v.precio_descuento || '', stock: v.stock || '0'
    })) : []
    setVariantesEditar(vars)
  }
  ...
  async function guardarEdicion(e: any) {
    ...
    form.append('variantes', JSON.stringify(variantesEditar))
    if (imagenEditar) {
    ...
    if (res.ok) { setEditando(null); setVariantesEditar([]); cargarProductos(token!) }
    setCargando(false)
  }
  ...
  async function registro(e: any) {
    ...
    setRegError('')
    setRegMensaje('')
    if (regPassword !== regConfirmar) { setRegError('Las contraseñas no coinciden'); return }
    if (regPassword.length < 6) { setRegError('La contraseña debe tener mínimo 6 caracteres'); return }
    setRegCargando(true)
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setRegError(data.error || 'Error al registrarse') }
      else {
    ...
    } catch { setRegError('No se pudo conectar al servidor') }
    ...
    <main className="min-h-screen bg-black">
      <div className="flex">
        <aside className="w-56 min-h-screen bg-gray-950 border-r border-gray-800 p-4 flex flex-col gap-1 fixed">
          ...
          <button onClick={() => { setEditando(null); setVariantesEditar([]); setMostrarListaProductos(true) }}
        ...
        <div className="ml-56 px-6 py-8 max-w-4xl mx-auto w-full">
          <div className="w-full max-w-lg bg-gray-900 rounded-2xl p-6 border border-gray-800 space-y-4">
            ...
            <form onSubmit={guardarEdicion} className="space-y-4">
              ...
              <input placeholder="Precio base *" type="number" value={editando.precio} onChange={e => setEditando({ ...editando, precio: e.target.value })}
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gray-500" required />
              ...
              {/* VARIANTES */}
              <VariantesForm variantes={variantesEditar} setVariantes={setVariantesEditar} />
                    ...
                    {editando.imagenes.map((img: any) => (
                      <div key={img.id} className="relative">
                        ...
                        <button type="button" onClick={() => {
                          const eliminarArr = editando.imagenes_eliminar || []
                          setEditando({ ...editando, imagenes: editando.imagenes.filter((i: any) => i.id !== img.id), imagenes_eliminar: [...eliminarArr, img.id] })
              ...
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => { setEditando(null); setVariantesEditar([]); setMostrarListaProductos(true) }} className="flex-1 border border-gray-700 text-gray-400 font-bold py-3 rounded-xl hover:text-white transition">
                  Cancelar
                </button>
              </div>
            </form>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-gray-500 text-xs">
                            ...
                            {Array.isArray(p.variantes) && p.variantes.length > 0 && (
                              <span className="text-blue-400 ml-2">({p.variantes.length} variantes)</span>
                            )}
                          </p>
                        </div>
                      </div>
          ...
          {mostrarForm && (
            <form onSubmit={agregarProducto} className="bg-gray-900 rounded-2xl p-6 border border-gray-800 space-y-4">
              ...
              <textarea placeholder="Descripcion" value={descripcion} onChange={e => setDescripcion(e.target.value)}
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gray-500 resize-none" rows={3} />
              <input placeholder="Precio base *" type="number" value={precio} onChange={e => setPrecio(e.target.value)}
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-gray-500" required />
              ...
              {/* VARIANTES */}
              <VariantesForm variantes={variantes} setVariantes={setVariantes} />
                    ...
                    <div key={p.id} className="bg-gray-900 rounded-2xl border border-gray-800 p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          ...
                          {Array.isArray(p.variantes) && p.variantes.length > 0 && (
                            <span className="text-xs bg-blue-900 text-blue-300 px-2 py-0.5 rounded-full">🔀 {p.variantes.length} variantes</span>
                          )}
